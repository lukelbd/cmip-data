...
approach invokes 'wget' while former uses a less informative downloader.
"""
import builtins
import re
...
def _echo_database(database, group_facets, key_facets, printer=None, message=None):
    """
...
def _glob_files(path, pattern='*'):
    ...
    """
    files = sorted(  # have only seen .nc4 but this is in case
        file for ext in ('.nc', '.nc[0-9]')
        for file in path.glob(pattern + ext)
    )
    files_duplicate = []
    files_filtered = [
...
def _init_printer(prefix, backup=False, **constraints):
    """
...
def _init_database(source, facets, project=None, flagship_detect=False):
    """
...
def _write_script(
    ...
    **constraints
    ...
    prefix, count = re.subn(r'openId\n', f'openId={openid!r}\n', ''.join(prefix))
    if openid and not count:
        print('Warning: Failed to substitute in user openid.')
    code = '"$chksum" == "$(echo "$cached" | cut -d \' \' -f3)"'
    replace = r'"$(awk "\$3 == $chksum { print \$3 }" "$cached")"'
    suffix, count = re.subn(re.escape(code), replace, ''.join(suffix))
    if not count:
        print('Warning: Failed to repair chksum comparison line.')
    code = '"$(get_mod_time_ $file)" == $(echo "$cached" | cut -d \' \' -f2)'
    replace = r'"$(awk "$(_get_mod_time_ $file) >= \$2 { print \$2 }" "$file")"'
    suffix, count = re.subn(re.escape(code), replace, suffix)
    if not count:
        print('Warning: Failed to repair modification time comparison line.')
    center = _sort_lines(center)
...
def compare_files(path='~/data', remove=False):
    ...
    path = Path(path).expanduser()
    files_all, files_duplicate = _glob_files(path)
    files_duplicate = {file.name for file in files_duplicate}
    files_downloaded = {
    ...
    files_missing = sorted(files_scripts - files_downloaded - files_duplicate)
    ...
    print('Duplicate files:', count_facets(files_duplicate, ('model', 'variable')))
    files_remove = sorted((*files_extra, *files_duplicate))
    if remove and files_remove:
        response = input(f'Remove {len(files_remove)} files (y/[n])?').lower().strip()
        if response[:1] == 'y':
            for file in files_remove:
                print(f'Removing file: {file}')
                file = path / file
                file.unlink()
    return files_finished, files_missing, files_remove
...
def download_script(
    ...
    **constraints
    ...
    print = _init_printer('download', **constraints)
    if flagship_filter:
...
def filter_script(
    ...
    **constraints
    ...
    print = _init_printer('filter', **constraints)
    path = Path(path).expanduser() / 'unfiltered'
    ...
    database, group_facets, key_facets = _init_database(
