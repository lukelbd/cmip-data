#!/usr/bin/env python
"""
Filter CMIP5 and CMIP6 data as follows:
...
line_years = lambda line: tuple(
    int(date[:4]) for date in line.split('.')[0].split('_')[-1].split('-')
)
...
def wget_filter(table='Amon', exp='piControl', filter=None):
    """
    ...
    prefix, suffix = wget_lines(files[0], complement=True)

    # Collect all download lines for files
    lines = []  # final lines
    input = [line for file in files for line in wget_lines(file)]
    ...
    for model in sorted(filter):
        ...
        for ivar in vars:
            ...
            years.append(
                (min([date[0] for date in iyears]), max([date[1] for date in iyears]))
            )
        years = (max([date[0] for date in years]), min([date[1] for date in years]))

        # Finally iterate through lines and add them if they are within this date range
        ilines = []
        for line in mlines:
...
def wget_models(table='Amon', exp='piControl', filter=None):
    """
    ...
    for file in files:
        ...
        imodels = {line_model(line) for line in wget_lines(file)}
    ...
    for model in sorted(models_all):
        ...
        filtered = bool(filter and model not in filter)
...
if __name__ == '__main__':
    ...
    monthly = wget_models(month, exp=exps)
    for exp in exps:
        file = wget_filter(month, exp=exp, filter=monthly)
    if not steps:
        exit()

    # Run for daily data
    daily = wget_models(
        steps, exp='piControl', filter=(monthly if dayfilter else None)
    )
    file = wget_filter(steps, exp='piControl', filter=daily)
