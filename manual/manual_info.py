...
# NOTE: These results are copied and hardcoded into MODELS_INSTITUTIONS in internals.py
...
if institutes:
    ...
    debug.write('This was automatically generated by manual_checks.py\n')
    projects = {'cmip5': 'scratch', 'cmip6': 'scratch'}
    for project, base in projects.items():
        print(f'Reading {project.upper()} metadata...')
        base = Path(f'~/{base}').expanduser()
        path = f'{project}-piControl-amon'
        starts = []
        models = set(file.name.split('_')[2] for file in path.glob('ts_Amon_*'))
        for model in models:
